...
import { WHATSAPP_DEFAULT_NUMBER } from "@/lib/whatsapp-industrial-cta"
...
interface ProductFiltersProps {
  ...
  /** Etiqueta para el mensaje de WhatsApp (categoría o subcategoría · categoría); solo vista categoría */
  whatsappListingLabel?: string
}
...
export function ProductFilters({ 
  ...
  const categoryWhatsappUrl = useMemo(() => {
    if (!whatsappListingLabel?.trim()) return ""
    const body = tCatalog("whatsappCategory.message", { label: whatsappListingLabel.trim() })
    return createWhatsAppUrl(WHATSAPP_DEFAULT_NUMBER, body)
  }, [whatsappListingLabel, tCatalog])

  const trackCategoryWhatsAppClick = useCallback(() => {
    if (typeof window === "undefined") return
    const pageType = getWhatsAppPageContext(pathname).pageType
    window.dataLayer?.push({
      event: "whatsapp_click",
      whatsapp_click_page: pageType,
      whatsapp_click_source: "category_sidebar",
    })
  }, [pathname])
    ...
    <div className="space-y-6">
      ...
      {categoryName && (
        <div className="py-4 pb-6 border-b border-inxora-light-blue/30 dark:border-gray-700">
          ...
          {whatsappListingLabel && categoryWhatsappUrl && (
            <div className="mt-4 px-1">
              <Button
                asChild
                size="lg"
                className="w-full min-w-0 bg-[#25D366] hover:bg-[#20BD5A] text-white py-3 sm:py-4 text-xs sm:text-sm md:text-base border-0"
              >
                <a
                  href={categoryWhatsappUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={trackCategoryWhatsAppClick}
                  className="inline-flex w-full min-w-0 items-center justify-center gap-2"
                >
                  <svg className="h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                    <path d={WHATSAPP_ICON_PATH} />
                  </svg>
                  <span className="hidden md:inline truncate min-w-0">{tCatalog("whatsappCategory.cta")}</span>
                  <span className="md:hidden truncate min-w-0">{tCatalog("whatsappCategory.ctaShort")}</span>
                </a>
              </Button>
            </div>
          )}
