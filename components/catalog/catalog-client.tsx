...
export function CatalogClient({
  ...
  const brandWhatsappUrl = useMemo(() => {
    if (!hideBrandFilter || !pageTitle?.trim()) return ''
    const body = tCatalog('whatsappCategory.message', { label: pageTitle.trim() })
    return createWhatsAppUrl(WHATSAPP_DEFAULT_NUMBER, body)
  }, [hideBrandFilter, pageTitle, tCatalog])

  const trackBrandWhatsAppClick = useCallback(() => {
    if (typeof window === 'undefined') return
    const pageType = getWhatsAppPageContext(pathname ?? '').pageType
    window.dataLayer?.push({
      event: 'whatsapp_click',
      whatsapp_click_page: pageType,
      whatsapp_click_source: 'brand_header',
    })
  }, [pathname])
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      <div className="w-full px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 py-6 sm:py-8">
        ...
        <div className="mb-8 sm:mb-10">
          ...
          {hideBrandFilter && brandWhatsappUrl && (
            <div className="mt-5 sm:mt-6 max-w-md">
              ...
              >
                <a
                  ...
                  onClick={trackBrandWhatsAppClick}
